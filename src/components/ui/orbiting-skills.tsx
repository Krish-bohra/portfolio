...
const OrbitingSkill = memo(({ config, angle }: OrbitingSkillProps) => {
  ...
  return (
    ...
    >
      <div
        className={`
          relative w-full h-full p-2 bg-[#121212]/90 backdrop-blur-md
          rounded-2xl flex items-center justify-center border border-white/5
      ...
      >
        ...
        {isHovered && (
          <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/95 border border-white/10 backdrop-blur-sm rounded-lg text-xs font-bold tracking-wider uppercase text-white whitespace-nowrap pointer-events-none drop-shadow-md">
            {label}
          </div>
        )}
      </div>
...
export default function OrbitingSkills() {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center overflow-visible">
      <div 
        className="relative w-[340px] h-[340px] md:w-[400px] md:h-[400px] flex items-center justify-center"
        onMouseEnter={() => setIsPaused(true)}
        onMouseLeave={() => setIsPaused(false)}
      >
        
        {/* Central "Code" Icon with enhanced glow */}
        <div className="w-20 h-20 bg-gradient-to-br from-[#1A1A24] to-[#0D0D14] rounded-full flex items-center justify-center z-10 relative shadow-2xl border border-white/5 backdrop-blur-xl">
          <div className="absolute inset-0 rounded-full bg-cyan-500/20 blur-xl animate-pulse cursor-pointer"></div>
